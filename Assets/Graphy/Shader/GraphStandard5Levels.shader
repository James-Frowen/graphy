Shader "Graphy/Graph Standard"
{
    Properties
    {
        ...
        _ExcellentColor("Excellent Color", Color) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 _ExcellentColor;
            fixed4 _GoodColor;
            fixed4 _NormalColor;
            fixed4 _CautionColor;
            fixed4 _CriticalColor;

            fixed _ExcellentThreshold;
            fixed _GoodThreshold;
            fixed _NormalThreshold;
            ...
            void CheckBar(inout fixed4 color, float yCoord, float barValue, fixed4 barColor)
            {
                if (yCoord < barValue && yCoord > barValue - 0.02)
                {
                    color = barColor;
                }
            }
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                if (graphValue > _ExcellentThreshold)
                {
                    color *= _ExcellentColor;
                }
                else if (graphValue > _GoodThreshold)
                {
                    color *= _GoodColor;
                }
                else if (graphValue > _NormalThreshold)
                {
                    color *= _NormalColor;
                }
                ...
                // Average white bar
                CheckBar(color, yCoord, Average, fixed4(.9, .8, .7, 1));

                // Threshold Bars
                CheckBar(color, yCoord, _ExcellentThreshold, _ExcellentColor);
                CheckBar(color, yCoord, _GoodThreshold, _GoodColor);
                CheckBar(color, yCoord, _NormalThreshold, _NormalColor);
                CheckBar(color, yCoord, _CautionThreshold, _CautionColor);
